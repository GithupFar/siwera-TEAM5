<?php
	...
	include("_conn.php");
	$sql = "select * from fp_article order by id_judul desc";
...
text-overflow: ellipsis;
  ...
  <body class="bg-secondary">
    ...
    <div class="container">
		<a href="insert.php"><button name="create" class="btn btn-primary" id="create">Upload</button></a>
      <!-- /.container -->
		  ...
		  <tbody style="color: white">
			  <?php $i = 1;?>
                ...
                <tr>
                    ...
                    <td><img src="../upload/<?php echo $value['foto'] ?>" height="200" max-width="500%" >
					</td>
                    <td><?php echo $value['judul'] ?></td>
                    <td ><?php echo $value['teks'] ?></td>
					<td><?php echo $value['lokasi'] ?></td>
                    <td style="text-align:center;">
                        <form action="db_edit.php" method="get">
                            <input type="hidden" name="id_judul" value="<?php echo $value['id_judul']?>">
							<input type="hidden" name="judul" value="<?php echo $value['judul']?>">
							<input type="hidden" name="foto" value="<?php echo $value['foto']?>">
                            <input type="hidden" name="teks" value="<?php echo $value['teks']?>">
							<input type="hidden" name="lokasi" value="<?php echo $value['lokasi']?>">
                            <button type="submit" name="edit" id="edit" class="btn btn-warning">Edit</button>
                        </form>
						<br />
                        <form action="db_delete.php" method="post">
                            <input type="hidden" name="id_judul" value="<?php echo $value['id_judul']?>">
                            <button type="submit" name="del_admin" id="del_admin" class="btn btn-danger" onClick="return confirm('Apakah anda ingin menghapus data ?')">Delete</button>
                        </form>
                    </td>
                </tr>
                <?php } ?>
